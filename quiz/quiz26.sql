...
USING (
    SELECT create_date AS std_date 
    FROM TABLE_DATA_1
    GROUP BY create_date
        ) D1
ON (C.std_date = D1.std_date)
WHEN MATCHED THEN
    UPDATE SET C.CHECK_DATA1 = 'Y'
WHEN NOT MATCHED THEN
    INSERT (std_date, CHECK_DATA1, CHECK_DATA2)
    VALUES (D1.std_date, 'Y', 'N');
...
ON (C.std_date = D2.std_date)
WHEN MATCHED THEN
    UPDATE SET C.CHECK_DATA2 = 'Y'
WHEN NOT MATCHED THEN
    INSERT (std_date, CHECK_DATA1, CHECK_DATA2)
    VALUES (D2.std_date, 'N', 'Y');

SELECT TO_CHAR(std_date, 'YY/MM/DD') AS STD_DATE, CHECK_DATA1, CHECK_DATA2
FROM TABLE_COLC
ORDER BY std_date;
