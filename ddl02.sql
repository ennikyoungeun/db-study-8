...
values('name4', null, TO_DATE('2020-01-04'));
...
NOT NULL : null 값저장 불가(Nullable NO)

무결성( DB 가장 기본 내용 :중복x + NOT NULL) : 데이터 올바르게, 일관되게,유지
    >정확성 :실제 값 올바르게 저장
        키 저장 172.4 -> 1724    X
        생일 1995-10-30 -> 1998-20-30 X
    >일관성 : 같은 의미 데이터는 일관된 값 (데이터가 동일)
        식당 포스기 : 15,000원
        키오스크 주문내역 : 13,000원
    >완전성 : 필요 데이터가 존재
            회원가입시 전화번호 인증 필수 : 전화번호 = NULL  ->X
            배달주문 메뉴내역 O, 주문자 정보X : 필수인 주문자 정보가 없다! ->X
    >신뢰성 : 데이터에 대한 믿음 신뢰
            계좌잔액10만원 -> 잔액조회시 계산오류로 8만원 표시 X
            신방 구매 재고확인 1개 -> 창고에 가지러 갔더니 0개 X
...
INSERT INTO t1 (no, birth) --생략한 name 컬럼에는 null값이 저장될 예정
...
123.456 O 10.551 O 1234.560 X <- NUMBER(7,3)
...
BOOLEAN True / false   --db에 블린 타입이 없다. -> CHECK 사용
    'TRUE' /'FALSE'
    'Y'/'N'
    'T'/'F'   where 컬럼 = 'T'
    1/0
